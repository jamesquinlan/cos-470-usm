...
\begin{outline}[enumerate]
    ...
    We can show this by confirming the 4 conditions of a distance metric:
        ...
        \2 $d(\vec{x},\vec{y}) \ge 0$: This is \textbf{true} as the range of the square root function is $[0,\infty)$.
        \2 $d(\vec{x},\vec{y}) = 0$ iff $x=y$. This is \textbf{not true}. If $\vec{x}=\vec{y}$, then we have $\sqrt{2|\vec{x}|} \neq 0$.
        \2 $d(\vec{x},\vec{y}) = d(\vec{y},\vec{x})$. This is \textbf{true}, as $|\vec{x}| + |\vec{y}| = |\vec{y}| + |\vec{x}|$.
        \2 $d(\vec{x},\vec{z}) \leq d(\vec{x},\vec{y}) + d(\vec{y},\vec{z})$. Let's not bother; we already proved this false with number 2. \\
